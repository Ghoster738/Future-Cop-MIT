...
namespace {
const uint8_t BITFIELD_RETRIGGER       = 0x02;
const uint8_t BITFIELD_ACTION_BUTTON   = 0x04;
const uint8_t BITFIELD_DISABLE_TRIGGER = 0x10;
}
...
bool Data::Mission::ACT::Trigger::readACTType( uint_fast8_t act_type, Utilities::Buffer::Reader &data_reader, Utilities::Buffer::Endian endian ) {
    ...
    internal.length = data_reader.readU16( endian ); // Length Values: 40, 81, 122, 204, 286, 327, 409, 512, 614, 737, 819, 1024, 1228, 1433, 1638, 1843, 2048, 2150, 2252, 2457, 2662, 2867, 3276, 3686, 4096, 4505, 4915, 5120, 5734, 6144, 6307, 6553, 7372, 7577, 8192, 8601, 9011, 9216, 9420, 9830, 10240, 10649, 11264, 11468, 12288, 14336, 16384, 20480, 24576, 28672,
    internal.height = data_reader.readU16( endian ); // Unknown Values: 0, 20, 40, 81, 102, 204, 409, 512, 614, 819, 1024, 1228, 1310, 1433, 1638, 2048, 2211, 2252, 2457, 3276, 3481, 4096, 4505, 5120, 8192, 12288, 16384,
    internal.uint8_0 = data_reader.readU8(); // Unknown Values: 0, 1, 3,
    internal.bitfield = data_reader.readU8(); // Bitfield Values: 0, 1, 2, 3, 4, 6, 16, 18, 22, 64, 66,
    internal.triggering_actor_id = data_reader.readU16( endian ); // Identifier Values: 7, 11, 12, 16, 17, 18, 19, 23, 32, 33, 42, 43, 52, 57, 73, 75, 77, 79, 82, 84, 85, 87, 111, 144, 154, 168, 172, 173, 185, 204, 205, 206, 211, 229, 245, 254, 255, 271, 298, 303, 307, 314, 325, 343, 349, 358, 432, 450, 455, 462, 476, 585, 611, 616, 623, 626, 628, 738, 65535,
