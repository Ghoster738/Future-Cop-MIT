...
Graphics::SDL2::GLES2::Internal::SkeletalModelDraw::SkeletalAnimation::SkeletalAnimation( unsigned int num_bones, unsigned int amount_of_frames ) {
    ...
    bone_frames.resize( amount_of_frames, nullptr );

    for( unsigned int i = 0; i < bone_frames.size(); i++ )
    bone_frames[ i ] = new Utilities::DataTypes::Mat4 [ num_bones ];
}

Graphics::SDL2::GLES2::Internal::SkeletalModelDraw::SkeletalAnimation::~SkeletalAnimation() {
    // Delete the joint matrix frames.
    for( unsigned int i = 0; i < bone_frames.size(); i++ )
    delete [] bone_frames[ i ];
}

Utilities::DataTypes::Mat4* Graphics::SDL2::GLES2::Internal::SkeletalModelDraw::SkeletalAnimation::getFrame( unsigned int current_frame ) {
    if( current_frame < bone_frames.size() )
    return bone_frames[ current_frame ];
    else
    return nullptr;
}
...
int Graphics::SDL2::GLES2::Internal::SkeletalModelDraw::inputModel( Utilities::ModelBuilder *model_type, int index, const std::vector<Texture2D*> textures ) {
    ...
    {
        auto frame_r = model_animation[ index ]->getFrame( frame_index );
        auto frame_cp_r = model_type->getJointFrame( frame_index );

        for( unsigned int bone_index = 0; bone_index < model_type->getNumJoints(); bone_index++ )
        {
        frame_r[ bone_index ] = frame_cp_r[ bone_index ];
        }
    }
    ...
    for( unsigned int d = 0; d < model_array.size(); d++ ) // Go through every model that has an instance.
    {
        ...
        if( mesh != nullptr && animate != nullptr )
        {
            ...
            for( auto instance = model_array[ d ]->instances.begin(); instance != model_array[ d ]->instances.end(); instance++ )
            {
                ...
                assert( animate->getFrame( current_frame ) != nullptr );

                glUniformMatrix4fv( mat4_array_uniform_id, animate->getNumBones(), GL_TRUE, reinterpret_cast<const GLfloat*>( animate->getFrame( current_frame ) ) );
