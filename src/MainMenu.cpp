...
void MainMenu::load( MainProgram &main_program ) {
    ...
    if( !this->is_game_on ) {
        ...
        this->items.emplace_back( new Menu::TextButton( "Announcement Player", glm::vec2( center, scale.y - 4 * step ), 0, 1, 2, 1, &item_click_player_announcement, prime_font, selected_font ) );
        this->items.emplace_back( new Menu::TextButton( "Options",             glm::vec2( center, scale.y - 3 * step ), 1, 2, 3, 2, &item_click_options,             prime_font, selected_font ) );
        this->items.emplace_back( new Menu::TextButton( "Exit to OS",          glm::vec2( center, scale.y - 2 * step ), 2, 3, 0, 3, &item_click_exit_game,           prime_font, selected_font ) );
