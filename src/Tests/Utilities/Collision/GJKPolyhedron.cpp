...
int testPolygon( std::string first_name, glm::vec3 position, std::vector<glm::vec3> cube_positon_data, std::vector<glm::vec3> cube_data ) {
    //
    int status = SUCCESS;
    
    std::string name = "[" + first_name + " x = " + std::to_string( position.x ) + ", y = " + std::to_string( position.y ) + ", z = " + std::to_string( position.z ) + "]";
    GJKPolyhedron cube( cube_positon_data );
    
    // Check support function for success.
    for( size_t i = 0; i < cube_data.size(); i++ )
        status |= testSupport( name, cube, cube_positon_data, i, glm::normalize(cube_data.at( i )), true );
    
    // Check support function for failure.
    for( size_t m = 0; m < cube_data.size(); m++ ) {
        for( size_t n = 0; n < cube_data.size(); n++ ) {
            if( m != n ) {
                status |= testSupport( name, cube, cube_positon_data, m, glm::normalize(cube_data.at( n )), false );
            }
        }
    }

    // Check if the center is where it is supposed to be.
    if( !glm::all( glm::equal(cube.getCenter(), position) ) ) {
...
int main() {
...

    {
        ...
        status |= testPolygon( name, center, generateCubeData( center ), generateCubeData() );
        ...
        for( int x = -1; x <= 1 && status == SUCCESS; x++ ) {
            for( int y = -1; y <= 1 && status == SUCCESS; y++ ) {
                for( int z = -1; z <= 1 && status == SUCCESS; z++ ) {
                    ...
                    scale *= 4.0;
                    status |= testPolygon( name, scale, generateCubeData( scale ), generateCubeData() );
                }
            }
        }
    }
