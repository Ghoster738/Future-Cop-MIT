...
namespace  {
    ...
    std::vector<uint32_t> getNumbers( Random::RandomGenerator &gen, uint32_t number ) {
...
int main() {
    ...
    { // General Test.
        Random random( 0x5F93282D6FDEC );
        Random::RandomGenerator general = random.getGenerator();
    ...
    { // Initialization Test.
        ...
        Random random;
        Random::RandomGenerator first = random.getGenerator();
        Random::RandomGenerator second = random.getGenerator();

        std::vector<uint32_t> first_array  = getNumbers(  first, 64 );
        std::vector<uint32_t> second_array = getNumbers( second, 64 );

        Random random_z(0);
        Random::RandomGenerator first_z = random_z.getGenerator();
        Random::RandomGenerator second_z = random_z.getGenerator();

        std::vector<uint32_t> first_array_z  = getNumbers( first_z,  64 );
        std::vector<uint32_t> second_array_z = getNumbers( second_z, 64 );

        Random random_1(1);
        Random::RandomGenerator first_1 = random_1.getGenerator();
        Random::RandomGenerator second_1 = random_1.getGenerator();
